...
#include <iostream>
using namespace std;
enum StatusCode { SUCCESS, FAIL, UNDER_FLOW,
		OVER_FLOW, RANGE_ERROR,
		DUPLICATE_ERROR, NOT_PRESENT,
		ENTRY_INSERTED, ENTRY_FOUND,
		VISITED, UNVISITED };
...
//  抽象数据类型方法声明及重载编译系统默认方法声明:
	...
	DblNode<ElemType>* GiveHead(){ return head;}
	void EmptyHead(){head = NULL;}
	void PrintList();
};
...
template<class ElemType>
void DblLinkList<ElemType> ::PrintList() {
    DblNode<ElemType> *p;
    p = head->next;
